...
- domain synonym normalization is implemented through `NLP/taxonomy/domain_synonyms.json`
- normalized text is now used for routing, safety checks, and retrieval query rewriting
...
- [x] add domain dictionary and synonym normalization
- [x] add multilingual support
- [x] add monitoring dashboards
- [x] add low-confidence and high-risk review flows
- [x] add retriever and generation regression tests

Current implementation notes:

- runtime NLP output now includes a coarse language label (`en`, `fr`, `ar`, `unknown`) and logs it in `nlp_events`
- CI now enforces language detection quality and retriever+generation regression checks in addition to rerank and clarification gates
...
## 13. Implemented Milestones (April 2026)

- 2-stage intent routing (coarse then fine) implemented in `NLP/pipeline/nlp.py`
- independent safety detector implemented in `NLP/pipeline/safety.py`
- hard-negative intent overrides implemented for recurring confusion phrases
- domain synonym normalization implemented in preprocessing
- evaluator now outputs per-intent confusion diagnostics and per-safety-flag metrics
- live low-confidence clarification flow implemented in `backend/routes/chat.py`
- clarification KPI signals now logged per message (`clarification_requested`, `clarification_follow_up`, `clarification_resolved`)
- ambiguity benchmark and clarification quality gate implemented in `NLP/evaluation/run_clarification_eval.py`
- production clarification KPI reporting implemented in `backend/scripts/report_clarification_kpis.py`
- hybrid retrieval reranking implemented in `backend/utils/rag_pipeline.py` via `NLP/pipeline/reranker.py`
- retrieval reranking benchmark and CI quality gate implemented in `NLP/evaluation/run_retrieval_rerank_eval.py`
- retrieval reranking benchmark expanded to 60-sample production set (`NLP/datasets/eval_retrieval_rerank_v2.jsonl`) with stricter gate thresholds
- real production user-message pairs are now ingested into rerank benchmarking (`eval_retrieval_rerank_real_v1.jsonl`) and mixed into CI benchmark `eval_retrieval_rerank_v3.jsonl`
- human-review workflow added for real rerank labels (`human_verified`, reviewer metadata, queue + coverage scripts, informational CI coverage check)
